...
//En caso de que no esten los cargamos y si ya fueron cargados anteriormente los pasamos al array de productos
...
setTimeout(() => {
  ...
  cotizacion.querySelector('h3').textContent = `Cotizacion del dia : $${dls}`
}, 1000)
...
async function btnAgregar() {
  ...
  if (btnAdd.textContent == 'Agregar') {
    ...
    if (nombre.value == '' || newPrecio.value == '' || newStock.value == '') {
      p.textContent = `Faltan Completar algunos campos`
